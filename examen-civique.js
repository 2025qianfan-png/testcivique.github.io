...
function initAuthUI() {
    ...
    if (user) {
        ...
        if (welcomeMessage) welcomeMessage.textContent = user.name;
        ...
        if (roleMenuContainer) roleMenuContainer.innerHTML = '';
    ...
    try {
        ...
        if (!result.success) {
            ...
            showToast(
                ...
                currentLang === 'fr' ? `Bienvenue ${result.user.name}` : `欢迎 ${result.user.name}`,
                'success'
            );
        }
    } catch (error) {
        errorMessageSpan.textContent = currentLang === 'fr' ? 'Une erreur est survenue' : '发生错误';
        errorDiv.style.display = 'flex';
    } finally {
...
async function updateProfile(event) {
    ...
    if (!newPassword && !newEmail) {
        errorDiv.style.display = 'flex';
        errorMessage.textContent = currentLang === 'fr' ? 'Veuillez modifier au moins un champ' : '请至少修改一项';
        return;
    }
    ...
    if (newEmail) {
        ...
        if (!emailRegex.test(newEmail)) {
            errorDiv.style.display = 'flex';
            errorMessage.textContent = currentLang === 'fr' ? 'Veuillez entrer une adresse email valide' : '请输入有效的邮箱地址';
            return;
        }
    }
    ...
    if (newPassword && newPassword.length < 6) {
        errorDiv.style.display = 'flex';
        errorMessage.textContent = currentLang === 'fr' ? 'Le mot de passe doit contenir au moins 6 caractères' : '密码至少需要6个字符';
        return;
    }
    ...
    try {
        ...
        if (hasError) {
            ...
            if (newPassword && newEmail) {
                successMsg.textContent = currentLang === 'fr' ? 'Mot de passe et email mis à jour' : '密码和邮箱已更新';
            } else if (newPassword) {
                successMsg.textContent = translations[currentLang].profileSuccessMessage;
            } else if (newEmail) {
                successMsg.textContent = currentLang === 'fr' ? 'Email mis à jour' : '邮箱已更新';
            }
            initAuthUI();
        ...
        errorDiv.style.display = 'flex';
        errorMessage.textContent = currentLang === 'fr' ? 'Une erreur est survenue' : '发生错误';
    } finally {
...
function switchLanguage(lang) {
    ...
    // 更新顶部语言按钮（如果存在）
    const topFr = document.getElementById('langFr');
    const topZh = document.getElementById('langZh');
    if (topFr) topFr.classList.toggle('active', lang === 'fr');
    if (topZh) topZh.classList.toggle('active', lang === 'zh');
    
    // 更新导航栏语言按钮
    const navFr = document.getElementById('navLangFr');
    const navZh = document.getElementById('navLangZh');
    if (navFr) navFr.classList.toggle('active', lang === 'fr');
    if (navZh) navZh.classList.toggle('active', lang === 'zh');
    ...
    if (user) {
        const t = translations[currentLang];
        document.querySelectorAll('.user-dropdown span').forEach(span => {
    ...
    if (feedbackTitle) feedbackTitle.textContent = data.feedbackShowcaseTitle || '⭐ Avis des étudiants';
    const feedbackSubtitle = document.getElementById('feedbackShowcaseSubtitle');
    if (feedbackSubtitle) feedbackSubtitle.textContent = data.feedbackShowcaseSubtitle || 'Ce que nos étudiants pensent de nos formations';
    
    const navLogin = document.getElementById('navLogin');
    if (navLogin) {
        const span = navLogin.querySelector('span');
        if (span) span.textContent = data.navLoginText || (lang === 'fr' ? 'Connexion' : '登录');
    }
    
    const navSituation = document.getElementById('navSituation');
    if (navSituation) navSituation.textContent = data.navSituation || (lang === 'fr' ? 'Mises en situation' : '情景题专项');
    
    const footerSituationLink = document.getElementById('footerSituationLink');
    if (footerSituationLink) footerSituationLink.textContent = data.footerSituationLink || (lang === 'fr' ? 'Mises en situation' : '情景题专项');
...
function fixTestButtons() {
    ...
    if (heroBtn) {
        heroBtn.textContent = currentLang === 'fr' ? 'Commencer maintenant' : '立即开始';
    }
}

function toggleFAQAnswers(lang) {
    document.querySelectorAll('.faq-answer > div').forEach(container => {
        if (lang === 'fr') {
...
function renderFeedbacks() {
    ...
    displayFeedbacks.forEach(fb => {
        ...
        if (avg < 5 && fullStars < 5) starsHtml += '⯨';
        const remaining = 5 - fullStars - (avg < 5 && fullStars < 5 ? 1 : 0);
        for (let i = 0; i < remaining; i++) starsHtml += '☆';
        ...
        html += `
            <div class="feedback-card">
                ...
                ${fb.comment ? `<div class="feedback-card-comment"><i class="fas fa-quote-left"></i>${escapeHtml(fb.comment)}</div>` : ''}
            </div>
        `;
    });
    ...
    if (loadMoreBtn) {
        if (feedbackDisplayCount < allFeedbacks.length) {
            ...
            if (span) span.textContent = t.loadMoreText || 'Voir plus d\'avis';
        } else {
...
function updateFeedbackCount() {
    ...
    if (countEl) {
        countEl.textContent = `${allFeedbacks.length} ${translations[currentLang].feedbackCountLabel || 'avis'}`;
    }
}
...
async function loadStats() {
    try {
        ...
        const passRate = studentCount > 0 ? Math.round((studentCount - 2) / studentCount * 100) : 0;
        let avgScore = 0;
        try {
        ...
        if (successRateEl) successRateEl.textContent = passRate;
...
function copyWechat() {
    navigator.clipboard.writeText('qfparis').then(() => {
        showToast(
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 7. 延迟加载需要 Supabase 的功能
    setTimeout(() => {
        try {
            loadStats();
            loadFeedbacks();
        } catch (e) {
            console.log('Supabase功能加载延迟:', e);
        }
    }, 500);
});
